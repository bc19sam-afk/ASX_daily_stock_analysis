...
class NotificationSummaryFormatTestCase(unittest.TestCase):
    ...
    def test_daily_report_downgrades_technical_analysis_volume_commentary_when_snapshot_metrics_missing(self) -> None:
        service = self._build_service()
        service._report_summary_only = False
        result = self._build_result(
    ...
    def test_daily_report_preserves_technical_analysis_when_snapshot_metrics_available(self) -> None:
        service = self._build_service()
        service._report_summary_only = False
        result = self._build_result(
            technical_analysis="量比走强并放量突破，换手率提升",
        ...
        report = service.generate_daily_report([result], report_date="2026-03-30")
        self.assertIn("**综合**：量比走强并放量突破，换手率提升", report)
