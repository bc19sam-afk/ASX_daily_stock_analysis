...
class NotificationSummaryFormatTestCase(unittest.TestCase):
    ...
    def test_dashboard_summary_escapes_long_operation_advice_and_reason(self, mock_get_db) -> None:
        ...
        self.assertIn("加仓 · 中等仓位（约 18%）", report)
    ...
    def test_wechat_dashboard_separates_executed_recommended_and_simulated(self, mock_get_db) -> None:
        ...
        self.assertIn("加仓 · 中等仓位（约 18%）", wechat)
    ...
    def test_daily_report_includes_data_time_baseline_and_mixed_source_disclosure(self, mock_datetime) -> None:
        ...
        self.assertIn("技术基准日说明：本次技术判断基于 **2026-03-29 日线（收盘口径）**。", report)
        self.assertIn("新闻信息更新至：**2026-03-30 09:30**。", report)
    ...
    def test_data_baseline_discloses_mixed_daily_dates_instead_of_first_result_only(self, mock_datetime) -> None:
        ...
        self.assertIn("技术基准日说明：本次技术判断基于 **多只股票日线日期不一致（混合日期）**。", report)
    ...
    def test_email_send_to_all_without_groups_uses_default_receivers_for_inline_image(
        self,
    ) -> None:
        service = self._build_channel_service([NotificationChannel.EMAIL])
        service._email_config = {"receivers": ["default@example.com"]}
    ...
    def test_wechat_report_snapshot_regression(self, mock_get_db, mock_datetime) -> None:
        ...
        self.assertIn("## 🎯 2026-03-30 决策仪表盘", report)
        self.assertIn("价格口径披露：**0/2** 只使用实时价格，**0/2** 只使用最新收盘，**2/2** 只按收盘口径。", report)
        self.assertIn("🟢 **贵州茅台(600519)**: 加仓 · 中等仓位（约 16%） (AI补充: 区间交易 / 评分78)", report)
        self.assertIn("⚪ **五粮液(000858)**: 持有观察 · 中低仓位（约 8%） (AI补充: 持有观察 / 评分52)", report)
    ...
    def test_feishu_report_snapshot_regression(self, mock_get_db, mock_datetime) -> None:
        ...
        self.assertIn("• 标的：🟢 **贵州茅台 (600519)** | 今日主动作（确定性/未执行）：加仓 · 中等仓位（约 16%） |", feishu)
    ...
    def test_primary_action_stays_canonical_while_ai_commentary_remains_independent(self, mock_get_db) -> None:
        ...
        self.assertIn("| 🟢 **贵州茅台 (AAA)** | 建议新开仓 · 中等仓位（约 18%） | 可轻仓跟踪 · 评分 75 · 震荡上行 |", report)
        self.assertIn("| ⚪ **贵州茅台 (BBB)** | 持有观察 · 中等仓位（约 18%） | AI解读与确定性主动作存在方向冲突，已转为中性说明 · 评分 75 · 震荡上行 ⚠️(已抑制冲突态AI操作措辞) |", report)
        self.assertIn("| 🔴 **贵州茅台 (CCC)** | 清仓 · 中等仓位（约 18%） | 继续拿住 · 评分 75 · 震荡上行 |", report)
    ...
    def test_position_advice_fallback_escapes_pipe_for_markdown_table_and_feishu(self, mock_get_db) -> None:
        ...
        self.assertIn("- 空仓者怎么做：ADD | 目标仓位 18.00% | 模拟Δ 3,200.00 | 目标数量 N/A（确定性引擎未提供）", report)

        feishu = format_feishu_markdown(report)
        self.assertIn("空仓者怎么做", feishu)
    ...
    @patch("src.notification.get_db")
    def test_ai_english_bare_share_count_is_redacted_but_stop_condition_remains(self, mock_get_db) -> None:
        mock_get_db.return_value.get_portfolio_overview.return_value = {"cash": 100.0, "holdings": []}
        service = self._build_service()
        service._report_summary_only = False
        result = self._build_result(
            code="RGN.AX",
            name="REGION GP",
            operation_advice="100 shares at 2.30 if confirmed; stop below 2.26",
            dashboard={
                "core_conclusion": {
                    "one_sentence": "100 shares at 2.30 if confirmed; stop below 2.26",
                    "position_advice": {
                        "no_position": "100 shares at 2.30 if confirmed; stop below 2.26",
                    },
                },
                "battle_plan": {
                    "position_strategy": {
                        "entry_plan": "100 shares at 2.30 if confirmed; stop below 2.26",
                        "risk_control": "stop below 2.26",
                    }
                },
            },
        )
        ...
        combined = "\n".join([report, single, wechat])
        self.assertNotIn("100 shares", combined)
        self.assertIn("stop below 2.26", combined)
    ...
    def test_per_stock_close_position_renders_zero_target_quantity_as_deterministic(self, mock_get_db) -> None:
        ...
        report = service.generate_dashboard_report([result], report_date="2026-03-30")
        self.assertIn("- 空仓者怎么做：CLOSE | 目标仓位 0.00% | 模拟Δ -3,200.00 | 目标数量 0 股", report)
