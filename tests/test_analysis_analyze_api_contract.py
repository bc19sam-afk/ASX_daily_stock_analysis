...
def test_analyze_rejects_blank_stock_codes_after_normalization():
    client = _build_client()
    ...
    payload = response.json()
    assert payload["error"] == "validation_error"
    assert "有效的 stock_code" in payload["message"]
...
def test_analyze_deduplicates_common_asx_aliases_before_single_stock_validation(monkeypatch):
    client = _build_client()
    captured = {}

    def _fake_sync_handler(stock_code, request):
        captured["stock_code"] = stock_code
        captured["request_stock_code"] = request.stock_code
        captured["request_stock_codes"] = request.stock_codes
        return AnalysisResultResponse(
            query_id="test_query_id",
            stock_code=stock_code,
    ...
    assert response.status_code == 200
    assert captured["stock_code"] == "NHF.AX"
    assert captured["request_stock_code"] == "NHF.ASX"
    assert captured["request_stock_codes"] == ["NHF.AX"]
