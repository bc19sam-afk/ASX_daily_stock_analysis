...
def test_market_symbol_classifier_supports_real_us_formats():
    assert DataFetcherManager._is_au_us_symbol("CBA.AX") is True
    assert DataFetcherManager._is_au_us_symbol("AAPL") is True
    assert DataFetcherManager._is_au_us_symbol("BRK.B") is True
    assert DataFetcherManager._is_au_us_symbol("BRK-B") is True
    assert DataFetcherManager._is_au_us_symbol("600519") is False


def test_resolve_enabled_fetchers_default(monkeypatch):
    monkeypatch.delenv("ENABLED_FETCHERS", raising=False)
    assert DataFetcherManager._resolve_enabled_fetchers() == ["yfinance"]


def test_resolve_enabled_fetchers_from_env(monkeypatch):
    monkeypatch.setenv("ENABLED_FETCHERS", "yfinance,tushare")
...
def test_asx_stock_routes_to_yfinance_only():
    ...
    _, source = manager.get_daily_data("CBA.AX", days=5)
