...
def _workbench_client(
    tmp_path: Path,
    *,
    stock_list: str | None = "BHP.AX,CBA.AX",
    ...
    if stock_list is not None:
        config_items.insert(0, {"key": "STOCK_LIST", "value": stock_list, "raw_value_exists": bool(stock_list)})
    app.dependency_overrides[get_system_config_service] = lambda: SimpleNamespace(
...
def test_presets_endpoint_returns_stable_review_only_contract(tmp_path: Path):
    ...
    assert payload["links"]["workbench"] == "/api/v1/workbench/summary"
    assert payload["schema"]["required_fields"] == sorted(REQUIRED_FIELDS)
    assert payload["schema"]["dry_run_payload_fields"] == [
        "name",
        "target_scope",
        "target",
        "alert_type",
...
def test_workbench_disables_watchlist_preset_without_configured_stock_list(tmp_path: Path):
    client, app = _workbench_client(tmp_path, stock_list=None)
    history_list = {
        "total": 1,
        "items": [
            {
    ...
    assert response.status_code == 200
    presets = response.json()["alert_rule_dry_run"]["presets"]
    watchlist = next(item for item in presets if item["id"] == "watchlist_data_basis_review")
    assert watchlist["enabled"] is False
    assert "STOCK_LIST" in watchlist["disabled_reason"]
    assert watchlist["payload"] == {
        "name": "Watchlist data basis review dry-run",
        "target_scope": "watchlist",
        "target": "all",
        "alert_type": "stale_price",
        "severity": "warning",
        "parameters": {"max_targets": 50},
    }
    app.dependency_overrides.clear()


def test_workbench_disables_context_dependent_presets_without_inputs(tmp_path: Path):
    client, app = _workbench_client(tmp_path, holdings=[])
    ...
    latest_data_gap = next(item for item in presets if item["id"] == "latest_report_data_gap")
    concentration = next(item for item in presets if item["id"] == "portfolio_concentration_review")
    price_stale = next(item for item in presets if item["id"] == "portfolio_price_stale_review")

    assert latest_validation["enabled"] is False
    assert latest_data_gap["enabled"] is False
    assert "No latest report symbol" in latest_validation["disabled_reason"]
    assert concentration["enabled"] is False
    assert price_stale["enabled"] is False
    assert "No portfolio holdings" in concentration["disabled_reason"]
    assert "No portfolio holdings" in price_stale["disabled_reason"]
    app.dependency_overrides.clear()
