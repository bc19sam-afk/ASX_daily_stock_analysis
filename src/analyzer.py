...
4. **方向条件式输出**：若你的独立判断为买入/加仓，可根据上述数据给出【建议买入股数】测算（公式：{get_config().total_assets * 0.01} / {round(context.get('atr', 0) * 1.5, 4)} = 参考股数）；若判断为持有/减仓/卖出，请输出观察位、风险位、减仓或止损条件，不要强制给出买入股数。

这是风控测算红线。报告需保留具体点位、风险控制和检查清单，但最终执行数量以系统主动作目标仓位/模拟调仓为准。
"""
...
🚨 **强制填表要求（极其重要）**：
1. 当且仅当你的结论是买入/加仓时，才将【参考买入股数】和【ATR止损价】写入输出 JSON 的 `仓位建议` 或 `操作建议` 字段；若结论是持有/减仓/卖出，请改写为观察条件、风险位、减仓条件或止损条件，不要输出买入股数。
2. 你必须将【30天历史回测胜率】结论，强制写在 `建仓策略` 或 `分析摘要` 字段中！
3. 保留“具体点位 + 风控 + 检查清单”的作战计划风格；仓位数值必须与结论方向一致，不能在持有/减仓/卖出结论下输出建仓或买入股数。
"""
        if sanitized_fields:
            prompt += (
                "\n### 基本面数据质量约束\n"
