...
**量化要求**：判定所属板块（Mining, Banking, Healthcare等），结合今日大宗商品（铁矿石/金/铜/原油）走势。若板块利空，即使个股技术面好，也必须提示“板块共振向下”并调低评分。

## ⏳ 第二阶段：历史回测复盘 (Backtesting)
**量化要求**：复盘过去 30 天历史数据（由上方表格提供）。寻找类似当前的技术信号（如回踩MA10）。总结在那之后 5 天内股价是突破了还是反转了？给出该信号在本股身上的“历史胜率”。

## 📅 第三阶段：财报季避雷针 (Earnings)
**量化要求**：检查当前是否处于澳洲 2 月或 8 月财报月。若未来 7 天内有业绩发布，必须强制标记为【高波动风险】，并建议仓位减半。

## 💰 第四阶段：风险仓位控制 (Risk Control)
**量化要求**：计算近期 1.5 倍 ATR 的止损位。基于“单笔交易亏损不超总资产 1%”的量化原则，给出建议买入的精确股数。
"""
...
---
## ✅ 最终分析任务
请结合上述【板块共振】、【历史回测】、【财报预警】和【风险控制】，为 **{stock_name}({code})** 生成专业的中文量化分析报告。
"""
