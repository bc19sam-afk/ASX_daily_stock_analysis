...
"""
        if news_context:
            prompt += f"""
### 🌍 量化第一阶段：板块轮动与共振 (Sector Check)
**在分析个股前，请你必须先执行以下步骤：**
1. 判定该股所属板块（Mining, Banking, Healthcare, REITs 等）。
2. 结合今日大盘复盘中的大宗商品（金/铜/铁矿石/原油）价格走势。
3. **核心准则**：如果所属板块整体受宏观利空（如铁矿石价格暴跌），即使个股技术面不错，也必须在结论中下调推荐等级，提示“板块共振向下”。

### 📰 舆情情报总结
以下是 **{stock_name}({code})** 近期的英文新闻与市场情报，请你直接用**中文**提取并总结：
1. 🚨 **价格敏感公告 (Price Sensitive)**：是否有突发监管审查、重大项目延期、大宗商品价格暴跌影响。
2. 🎯 **分红与资本管理**：最新宣布的股息派发计划、红利抵免（Franking）比例、股票回购。
3. 📊 **2026年2月财报季追踪**：最新财报的核心数据（净利润、营收）、是否超出机构预期（Beat/Miss）。
4. 🌍 **宏观大盘联动**：提取新闻中提及的美股隔夜表现、ASX 200 大盘情绪或大宗商品周期对该股的直接影响。
