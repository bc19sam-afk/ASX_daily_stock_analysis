...
class PositionManager:
    ...
    def decide(
        ...
        *,
        ...
        # 现金约束：加仓金额不能超过可用现金
        delta_amount = round(delta_weight * total_value, 2)
        if delta_amount > available_cash and total_value > 0:
            max_delta_weight = available_cash / total_value
            delta_weight = min(delta_weight, max_delta_weight)
            target_weight = round(current_weight + delta_weight, 4)
            delta_amount = round(delta_weight * total_value, 2)
